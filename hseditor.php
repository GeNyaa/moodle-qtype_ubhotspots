...
//require_once($CFG->libdir.'/form/htmleditor.php');
...
<head>
    ...
    var accordionHeader = "<h3 id=\"ahid_idname_\"><a href=\"#\">hotspot _idname_</a><span style=\"position: absolute; right: 2px; top: 2px; background-color: _color_;width:35px;height:35px\"></span></h3>";
    ...
    $(document).ready(function () {
        var res = jQuery.post('<?php echo $CFG->wwwroot; ?>/repository/draftfiles_ajax.php?action=list', {
            sesskey: getURLParameter('sesskey'),
            filepath: '/',
            itemid: getURLParameter('imageid')
        }, null, 'json').done(function (data) {
            if (data.filecount == 1) {
                backgroundImgPath = data.list[0].url;
                initCanvas();
            } else {
    ...
    function initLayout() {
        function addElementAcc() {
            ...
            $("#hst" + hotSpotsCounter).click(function () {
                ...
                $('#addtextdialog').dialog({
                    ...
                    autoOpen: false,
                    buttons: { "Ok": function () {
                        $("#" + hsid).html(tinymce.get('hsaddtext').getContent());
                        tinymce.get('hsaddtext').destroy();
                        $(this).dialog("close");
                    }
                    }
                }).dialog('open');
        ...
        $("#bclose").button({ icons: {primary: 'ui-icon-circle-close'}}).click(function () {
            $("<p>" + getString('closewithout') + "</p>").dialog({title: getString('warning'), modal: true, autoOpen: false, buttons: { "OK": function () {
                self.close();
            }, "No": function () {
                $(this).dialog("close");
            }}}).dialog('open');
        ...
        $('#c2').on('mousedown', function (e) {
            ...
            if (!currentHS || drawBuffer[currentHS].draw != null) return true;
            ...
            var mouseX = Math.round( (e.clientX - offset_l) );
            var mouseY = Math.round( (e.clientY - offset_t) );
